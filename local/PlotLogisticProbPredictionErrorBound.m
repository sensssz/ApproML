function PlotLogisticProbPredictionErrorBound( original_model, probabilities, models, error_bounds, testf )
  PlotBoundingProbabilities(original_model, probabilities, models, error_bounds, testf);
  ...
  PlotVisualErrorBound(original_model, probabilities, models, error_bounds, tests);
end

function PlotBoundingProbabilities( original_model, probabilities, models, error_bounds, testf )
...
function PlotVisualErrorBound( original_model, probabilities, models, error_bounds, testf )
  num_probabilities = size(probabilities, 2);
  abs_testf = abs(testf);
  num_tests = size(testf, 1);
  ...
  xticklabels = cell(num_probabilities);
  xticklabels = xticklabels(1, :);
  for j = 1:num_probabilities
      xticklabels{j} = strcat(num2str(probabilities(1, j) * 100), '%');
  end
  set(gca,'FontSize',22);
  set(gca,'XTick',linspace((num_tests+1)/2, (num_tests+1)*num_probabilities-num_tests/2, num_probabilities));
  set(gca, 'xticklabel', xticklabels);
  xlabel('Sampling Rate');
